...
2. Run `node .github/skills/_shared/scripts/generate.js --spec-file plans/<project-name>/spec.json`.
3. Use `--dry-run` first when:
...
4. Apply template-specific customization only when `postGeneration.tasks` is non-empty.
5. Report the generated path, template, spec files written, and any remaining follow-up.

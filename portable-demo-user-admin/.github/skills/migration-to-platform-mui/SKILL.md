...
3. Write or update `plans/<task-id>/plan.md` and `plans/<task-id>/migration.json` before making non-trivial changes.
4. If the migration touches theme or provider setup, read [references/provider-adoption.md](references/provider-adoption.md) and reuse `mui-v6-design`.
5. If the migration touches icons, read [references/icon-migration.md](references/icon-migration.md).
6. If the migration replaces admin tables, read [references/mrt-migration.md](references/mrt-migration.md) and reuse `material-react-table`.
7. If the migration upgrades JavaScript to TypeScript, read [references/js-to-ts-incremental.md](references/js-to-ts-incremental.md).
8. Execute in phases: inventory, foundation, pilot, expand, cleanup.
9. Run `quality-gate` after meaningful migration phases and finish with `code-review` for risky or broad migrations.
